...
export default function SchedulePage() {
  ...
  useEffect(() => {
    api.get("/classes").then(({ data }) =>
      setClasses(data.filter((c: ClassDto) => c.isActive))
    ).catch(() => {});
  }, []);
  ...
  // Sessions per day (index 0=Mon…6=Sun)
  ...
  const openCreate = (dayIndex?: number) => {
    const defaultDate = addDays(weekStart, dayIndex ?? 0);
    defaultDate.setHours(9, 0, 0, 0);
    setCreateForm({
      classId: classes[0]?.id ?? "",
      startTime: localDatetimeValue(defaultDate),
      notes: "",
    });
    setCreateError("");
    setCreateOpen(true);
  ...
  const handleCancel = async (session: SessionDto) => {
    if (!confirm(`Cancel "${session.className}" on ${formatTime(session.startTime)}?`)) return;
    setActionLoading(true);
    try {
      const { data } = await api.patch(`/classes/sessions/${session.id}/cancel`);
  ...
  const handleDelete = async (session: SessionDto) => {
    ...
    setActionLoading(true);
    try {
      await api.delete(`/classes/sessions/${session.id}`);
  ...
  return (
    <AdminRoute>
        ...
        <main className="mx-auto max-w-7xl px-4 py-10">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={goToday}
                className="rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-600 shadow-sm transition hover:border-rose-200 hover:text-rose-400"
              >
                Today
              </button>
              <button onClick={prevWeek} className="rounded-xl border border-gray-200 bg-white p-2 shadow-sm transition hover:border-rose-200 hover:text-rose-400">
                <ChevronLeft size={18} />
              </button>
              <span className="min-w-[200px] text-center text-sm font-medium text-gray-700">{weekLabel}</span>
              <button onClick={nextWeek} className="rounded-xl border border-gray-200 bg-white p-2 shadow-sm transition hover:border-rose-200 hover:text-rose-400">
                <ChevronRight size={18} />
              </button>
              <button
                onClick={() => openCreate()}
                className="flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-medium text-white shadow-sm transition"
            ...
            <div className="grid grid-cols-7 border-b border-gray-100">
              {DAYS.map((day, i) => {
                ...
                return (
                  <div
                    key={day}
                    className="border-r border-gray-100 px-3 py-3 last:border-r-0 text-center"
                  >
                    <p className={`text-xs font-medium uppercase tracking-wide ${isToday ? "text-rose-400" : "text-gray-400"}`}>
                      {day}
                    </p>
                    <p className={`mt-0.5 text-sm font-semibold ${isToday ? "text-rose-500" : "text-gray-700"}`}>
                      {date.getDate()}
                    </p>
                  </div>
                );
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-7 min-h-[320px]">
                {sessionsByDay.map((daySessions, i) => (
                  <div
                    key={i}
                    className="border-r border-gray-50 last:border-r-0 p-2 flex flex-col gap-1.5"
                  >
                    {daySessions.map(s => (
                      <button
                        key={s.id}
                        onClick={() => setSelected(s)}
                        className={`w-full rounded-xl px-2.5 py-2 text-left text-xs transition hover:opacity-80 ${
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-2xl bg-white shadow-xl">
            <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
              ...
              <button onClick={() => setCreateOpen(false)} className="rounded-lg p-1.5 text-gray-400 hover:bg-gray-100 transition">
                <X size={18} />
              </button>
            </div>
            <form onSubmit={handleCreate} className="space-y-4 px-6 py-5">
              {createError && (
                <div className="rounded-xl bg-rose-50 border border-rose-100 px-4 py-3 text-sm text-rose-600">{createError}</div>
              )}
              ...
              <div className="flex justify-end gap-3 border-t border-gray-100 pt-4">
                ...
                <button
                  type="submit"
                  disabled={createSaving || classes.length === 0}
                  className="flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-medium text-white disabled:opacity-60 transition"
                  style={{ background: "linear-gradient(135deg, #fb7185 0%, #c084fc 100%)" }}
                >
      ...
      {selected && (
        <div
          className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:justify-end p-4 sm:pr-6"
        ...
        >
          <div className="w-full sm:w-80 rounded-2xl bg-white shadow-xl border border-gray-100">
            ...
            <div
              ...
              style={{ backgroundColor: selected.colorLabel }}
            />
            <div className="px-5 py-4">
              <div className="flex items-start justify-between">
                ...
                <button onClick={() => setSelected(null)} className="rounded-lg p-1 text-gray-300 hover:bg-gray-100 transition">
                  <X size={16} />
                </button>
              </div>
              ...
              <div className="mt-4 space-y-2 text-sm text-gray-600">
                <div className="flex justify-between">
                  ...
                  <span>{formatDate(new Date(selected.startTime))} · {formatTime(selected.startTime)}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span>{selected.currentEnrollment} / {selected.maxCapacity}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">Status</span>
                  <span
                    className={`font-medium ${
                      selected.status === "Scheduled" ? "text-emerald-600" :
                      selected.status === "Cancelled" ? "text-rose-500" : "text-gray-500"
                    }`}
                  >
                    {selected.status}
                  </span>
                ...
                {selected.notes && (
                  <div className="pt-1 text-gray-500 italic">&ldquo;{selected.notes}&rdquo;</div>
                )}
              </div>
