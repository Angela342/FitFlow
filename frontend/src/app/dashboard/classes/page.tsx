...
export default function ClassesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editTarget) {
        await api.put(`/classes/${editTarget.id}`, payload);
      } else {
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="mx-auto max-w-7xl px-6 py-10">
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={openCreate}
              className="flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-medium text-white shadow-sm transition"
                    ...
                    <tr key={c.id} className="group transition hover:bg-gray-50/70">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <span
                            ...
                            style={{ backgroundColor: c.colorLabel }}
                          />
                          <div>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition">
                          <button
                            onClick={() => openEdit(c)}
                            className="rounded-lg p-1.5 text-gray-400 hover:bg-rose-50 hover:text-rose-400 transition"
                          >
                            <Pencil size={15} />
                          </button>
                          <button
                            ...
                            className="rounded-lg p-1.5 text-gray-400 hover:bg-rose-50 hover:text-rose-500 transition disabled:opacity-40"
                          >
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    value={form.category}
                    onChange={handleChange}
                    className="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm text-gray-800 outline-none transition focus:border-rose-300 focus:ring-2 focus:ring-rose-100 bg-white"
                  >
                  ...
                  <div className="flex gap-2">
                    {COLOR_OPTIONS.map(opt => (
                      <button
                        key={opt.value}
                        type="button"
                        onClick={() => setForm(prev => ({ ...prev, colorLabel: opt.value }))}
                        title={opt.label}
                        className={`h-7 w-7 rounded-full transition ${
                          form.colorLabel === opt.value
